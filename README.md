...
- **AI Summarization & Deduplication:** Distills long articles into concise 300-word summaries and intelligently groups duplicate news across different sources.
- **Raw HTML Passthrough Mode:** If AI summarization is disabled for a topic, FeedMind directly passes through the original raw HTML from the RSS feed, preserving exact layouts and inline images.
- **AI Batch Processing:** When a massive influx of articles arrives (e.g., >20), FeedMind automatically splits them into batches to prevent the LLM from choking or timing out on excessively long contexts.
...
2. Go to **Settings > Secrets and variables > Actions** in your fork and add your repository secrets. Depending on your chosen `LLM_PROVIDER`, add `GEMINI_API_KEY` or `DEEPSEEK_API_KEY`. You can also add `LLM_PROVIDER` or `DEEPSEEK_MODEL`/`GEMINI_MODEL` as variables to override the defaults.
...
3.  Copy `.env.example` to `.env` and fill in your API keys:
    *   Set `LLM_PROVIDER` to `gemini` or `deepseek`.
    *   For Gemini: Get an API key from Google AI Studio and set `GEMINI_API_KEY`.
    *   For DeepSeek: Get an API key from DeepSeek Platform and set `DEEPSEEK_API_KEY`.
4.  Configure `config.yaml` to customize your topics, feeds, and whether to use AI summarization.

## Flexible LLM Support

The system uses a highly scalable Factory pattern, allowing you to choose between:
*   **Gemini** (Default): Uses the `google-genai` SDK.
*   **DeepSeek (OpenAI-compatible)**: Uses the `openai` SDK with JSON mode. Because it uses the OpenAI standard, you can theoretically connect it to any OpenAI-compatible endpoint (like Groq, vLLM, Ollama) by changing the `base_url`.
...
- **AI 智能总结与聚合：** 将长篇文章或长篇论坛帖子压缩提炼为 300 字的精华摘要，并跨信源智能去重聚合。
- **直通模式 (Raw HTML Passthrough)：** 支持针对特定主题关闭 AI 总结，此时系统会直接搬运原生 RSS 的原始 HTML 源码（保留所有原始排版和内联图片）。
- **智能分批处理 (Batch Processing)：** 当瞬间涌入大量新文章（如 >20 篇）时，系统会自动切块分批交由大模型处理，彻底杜绝因为上下文过长导致的 API 挂起或卡死问题。
...
**使用方法：**
1. Fork 本仓库。
2. 在你 Fork 的仓库中进入 **Settings > Secrets and variables > Actions**，添加相应的 Repository Secrets。根据你选择的 `LLM_PROVIDER`，添加 `GEMINI_API_KEY` 或 `DEEPSEEK_API_KEY`。你也可以按需添加 `LLM_PROVIDER`，`DEEPSEEK_MODEL`，或 `GEMINI_MODEL` 作为变量来覆盖默认值。
